...
# Normalized lookup dictionary (all keys lowercase with no spaces or hyphens)
NORMALIZED_LICENSE_MAP = {
    "ccby4.0": LicenseType.CC_BY_4_0,
    "ccby3.0": LicenseType.CC_BY_4_0,
    "ccby2.0": LicenseType.CC_BY_4_0,
    "ccbync4.0": LicenseType.CC_BY_NC_4_0,
    "ccbync3.0": LicenseType.CC_BY_NC_4_0,
...
def normalize_and_validate_license(license_raw: str) -> LicenseType:
    """Validate a raw license string against the open license allowlist.

    Raises MissingAttributionError if the license is not on the open allowlist.
    """
    if not license_raw or not license_raw.strip():
        raise MissingAttributionError("License string cannot be empty.")

    clean_key = license_raw.strip().lower().replace("-", "").replace(" ", "")

    if clean_key in NORMALIZED_LICENSE_MAP:
        return NORMALIZED_LICENSE_MAP[clean_key]

    for key, val in NORMALIZED_LICENSE_MAP.items():
        if key in clean_key:
            return val

    raise MissingAttributionError(f"License '{license_raw}' is not on the open allowlist.")
